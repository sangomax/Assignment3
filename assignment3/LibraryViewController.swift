...
class LibraryViewController: UIViewController {
    ...
    let rooms: UIImageView = {
        let image = UIImageView(image: UIImage(named: "library"))
        image.translatesAutoresizingMaskIntoConstraints = false
        return image
    }()
    
    let secretPassageButton: UIButton = {
    ...
    override func viewDidLoad() {
        ...
        buttonsArea.addArrangedSubview(secretPassageButton)
        buttonsArea.addArrangedSubview(UIView())
        buttonsArea.addArrangedSubview(UIView())
        
        secretPassageButton.addTarget(self, action: #selector(goToNextRoom(_:)), for: .touchUpInside)
    ...
    @objc func goToNextRoom(_ sender: UIButton) {
        if sender.currentTitle == "Secret Passage" {
            let secretPassageVC = SecretPassageViewController()
            navigationController?.pushViewController(secretPassageVC, animated: true)
        }
    }
